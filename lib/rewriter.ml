...
let rewrite ?sat:(sat=3) ?fgb:(fgb=true) (eqs : Expr.qexpr list) (ineqs : Expr.qexpr list) vars_to_keep (ts : Expr.qexpr list) : Expr.qexpr list = 
  ...
  let rewritten_ts = 
    if non_consts_ts = [] then List.map (fun (i, const_t) -> i, CongQ.unpurify const_t cl) const_ts
    else
      ...
      List.map (fun (i, t) -> i, ConeQ.unpurify t cone) (rewritten_non_consts @ const_ts)
  in
  List.map (fun (_, uns) -> Expr.simplify uns) (List.sort (fun (i, _) (j, _) -> compare i j) rewritten_ts)